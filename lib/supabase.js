...
export async function fetchPortfolioStats() {
  console.log('[Dashboard] fetchPortfolioStats called, supabase exists:', !!supabase)
  ...
  console.log('[Dashboard] fetchPortfolioStats result:', trades?.length, 'error:', error)
  console.log('[Dashboard] after result log, checking trades...')
  if (trades && trades.length > 0) {
    console.log('[Dashboard] trade 0 status:', trades[0].status, 'pnl:', trades[0].pnl)
    console.log('[Dashboard] trade 1 status:', trades[1]?.status, 'pnl:', trades[1]?.pnl)
    console.log('[Dashboard] all statuses:', trades.map(t => t.status))
  }
  console.log('[Dashboard] about to filter closed...')
  console.log('[Dashboard] checking each trade:')
  trades.forEach((t, i) => {
    console.log(`[Dashboard] trade ${i}: status="${t.status}" (type: ${typeof t.status}), comparing to "closed": ${t.status === 'closed'}`)
  })
  if (!trades) return { totalTrades: 0, totalPnl: 0, winRate: 0 }
  
  const closed = trades.filter(t => t.status === 'CLOSED_WIN' || t.status === 'CLOSED_LOSS' || t.status === 'closed' || t.status === 'CLOSED')
  console.log('[Dashboard] closed trades:', closed.length, 'statuses:', closed.map(t => t.status))
  const totalPnl = closed.reduce((sum, t) => sum + (parseFloat(t.pnl) || 0), 0)
  const wins = closed.filter(t => t.status === 'CLOSED_WIN' || t.status === 'closed' && parseFloat(t.pnl) > 0).length
  console.log('[Dashboard] wins:', wins, 'closed:', closed.length)
  const winRate = closed.length > 0 ? (wins / closed.length) * 100 : 0
  console.log('[Dashboard] calculated winRate:', winRate)
...
export async function closeTrade(tradeId, exitPrice, direction, entryPrice) {
  ...
  try {
    ...
    console.log('[Dashboard] Attempting to close trade with:', { exit_price: exitPriceNum, pnl: pnlValue, status: closedStatus })
