...
export async function fetchPortfolioStats() {
  ...
  console.log('[Dashboard] FORCE LOG - about to filter closed')
  const closed = trades.filter(t => {
    const isClosed = t.status === 'CLOSED_WIN' || t.status === 'CLOSED_LOSS' || t.status === 'closed' || t.status === 'CLOSED'
    console.log('[Dashboard] filter check:', t.id, 'status:', t.status, 'isClosed:', isClosed)
    return isClosed
  })
  console.log('[Dashboard] FORCE LOG - closed trades:', closed.length)
