...
export async function fetchLastClosedTrade() {
  if (!supabase) return null
  const { data } = await supabase
    .from('trade_logs')
    .select('*')
    .in('status', ['CLOSED_WIN', 'CLOSED_LOSS', 'closed'])
    .order('created_at', { ascending: false })
    .limit(1)
    .single()
  return data || null
...
export async function fetchLatestAIPrediction() {
  if (!supabase) return null
  const { data } = await supabase
    .from('ai_predictions')
    .select('*')
...
export async function fetchPortfolioStats() {
  ...
  const wins = closed.filter(t => t.status === 'CLOSED_WIN' || (t.status === 'closed' && parseFloat(t.pnl) > 0) || (t.status === 'CLOSED' && parseFloat(t.pnl) > 0)).length
